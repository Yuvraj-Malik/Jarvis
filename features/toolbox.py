...
def open_app(name):
    ...
    try:
        ...
        # Wrap name in quotes to handle spaces
...
# --- ADVANCED AUTOMATION (JARVIS 2.0) ---

def execute_gui_action(action, **kwargs):
    """
    Handles clicks, typing, and hotkeys.
    ...
    try:
        if action == "click":
            ...
            return "Click failed: missing coordinates."
            ...
            return "Hotkey failed: no keys provided."
            ...
            return "Press failed: no key provided."

        return f"GUI action '{action}' not recognized."
    except Exception as e:
...
def run_terminal_command(command):
    ...
    try:
        # Use shell=True to handle built-in commands like 'dir' or 'cls'
...
def search_windows_settings(query):
    """Opens Windows Search and types the query."""
    try:
        pyautogui.press('win')
        time.sleep(0.5)
        ...
        time.sleep(0.5)
        pyautogui.press('enter')
        return f"Searching for '{query}' in Windows."
    except Exception as e:
